...
export function ComboboxPositioner<T extends ValidComponent = "div">(
  rawProps: PolymorphicProps<T>,
): JSX.Element {
  const api = useComboboxApi();
  if (!api) {
    throw Error("Combobox.Positioner component should be used inside of a Combobox");
  }
  return <Polymorphic {...mergeProps({ as: "div" as T }, api().getPositionerProps(), rawProps)} />;
}
